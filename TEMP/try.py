...
#     tool_registry.register_tool(suggest_recipes_tool)
...
#     # Create a two-step orchestrator
#     # 1. First use the classifier agent
#     # 2. Then use the specialized agent
...
#         # Get conversation context to enhance routing
...
#         try:
#             # Create an orchestrator just for classification
#             classifier_orchestrator = SimpleOrchestrator(
#                 agent_registry=agent_registry,
#                 default_agent_name="classifier_agent"
#             )
            
#             # Get the classification (agent name)
#             agent_name = classifier_orchestrator.orchestrate(
#                 thread_id=thread_id,
#                 user_message=enriched_input
#             ).strip()
            
#             # Now create an orchestrator for the specific agent
...
#                 )
#             else:
#                 response = f"I'm not sure how to handle that request. (Classification: {agent_name})"
#                 print(response)
...
#         except Exception as e:
#             print(f"\nError: {str(e)}")
#             print("Let me try to help you with something else instead.")
...
def create_freshness_agent(tool_registry):
    """Create the freshness tracking agent."""
    agent_config = AzureOpenAIAgentConfig(
        agent_name="freshness_agent",
        description="Agent for tracking item expiry dates and freshness alerts",
        model_name="gpt-4o",
        agent_type="ChatAgent",
        tool_registry=tool_registry,
        system_prompt="""
        You are the Freshness Agent, responsible for tracking expiry dates of food items and 
        alerting users about items that need to be used soon. 
        
        You can:
        1. Track expiry dates for food items
        2. Alert users about items that will expire soon
        3. Provide suggestions for using items before they expire
